...
public class BinarySearchTree implements Iterable<BinarySearchTree.BSTNode> {
    ...
    private class BSTIterator implements Iterator<BinarySearchTree.BSTNode> {
        ...
        private Stack<BSTNode> stack; // top of stack will contain leftmost/min node

        private BSTIterator(BSTNode root) {
            stack = new Stack<>();
            pushLeft(root);
        }
        ...
        public BSTNode next() {
            if (!hasNext()) {
                throw new NoSuchElementException("No further in-order-successor-node in BST!");
            }
            BSTNode next = stack.pop();
            pushLeft(next.getRightChild()); // descendants of right child are next in line
            return next;
        }

        public void remove() {
            throw new UnsupportedOperationException("Operation not supported in current context!");
        }

        private void pushLeft(BSTNode node) {
            // from node to its leftmost descendant, push to stack
            while (node != null) {
                stack.push(node);
